...
Проект для анализа транзакций и выявления аномалий и мошенничества с использованием ELT-процессов.
...
Поток данных:
1. Python Generator → MongoDB (генерация транзакций каждые 60 сек)
2. transactions_el DAG → PostgreSQL (EL: Extract + Load RAW, каждые 30 мин)
3. dbt_transformations DAG → PostgreSQL (T: Transform в DBT, каждый час)
   - STG: парсинг JSON, очистка данных
   - ODS: нормализация
   - DWH: агрегация (incremental)
   - DM: бизнес-витрины
4. Grafana → PostgreSQL (визуализация метрик в реальном времени)
```
...
- **Генератор транзакций** - Python-сервис, генерирующий синтетические транзакции в MongoDB
- **MongoDB** - источник данных (операционная БД)
- **EL-процесс в Airflow** - Extract + Load RAW данных в PostgreSQL (без трансформации!)
- **DBT** - вся трансформация в SQL (STG -> ODS -> DWH -> DM)
- **Elementary** - мониторинг качества данных
- **Grafana** - дашборды для визуализации (логин: admin/admin)
...
│       └── api.py               # REST API для управления транзакциями
├── tests/                        # Тесты
│   └── test_collector.py
...
│   ├── dags/                    # DAG файлы
│   │   ├── transactions_el.py   # EL процесс (Extract + Load RAW)
│   │   └── dbt_pipeline.py      # DBT трансформации (STG->ODS->DWH->DM)
│   ├── logs/                    # Логи Airflow
│   └── plugins/                 # Плагины Airflow
├── dbt/                          # DBT проект
│   ├── models/                  # DBT модели (STG, ODS, DWH, DM)
│   ├── tests/                   # Кастомные тесты
│   └── profiles.yml             # DBT профили (не в Git)
...
- **Apache Airflow** - оркестрация EL и DBT процессов
- **DBT** - трансформация данных (STG -> ODS -> DWH -> DM)
- **Elementary** - мониторинг качества данных
...
6. Откройте Grafana:
   - URL: http://localhost:3001 (на сервере: http://158.160.186.46:3001)
   - **Username: `admin`**
   - **Password: `admin`**
   - Дашборд: BankShield Transactions

7. Откройте Elementary EDR (Data Quality Monitoring):
   - URL: http://localhost:8081 (на сервере: http://158.160.186.46:8081)
